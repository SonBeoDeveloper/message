...
import { useMutationHandler } from '@/hooks/use-mutation-handle';
...
const ProfileDialogContent = () => {
    ...
    const { mutate: updateState, state: updateStatusState } = useMutationHandler(api.status.update)
    ...
    async function updateStatusHandler() {
        try {
            await updateState({ clerkId: user?.id!, status })
    ...
    return (
        <div>
            ...
            <div className="flex flex-col gap-y-6 mt-4">
                ...
                <Separator />
                <Dialog open={friendRequestModal} onOpenChange={() => setFriendRequestModal(!friendRequestModal)}>
                    <DialogTrigger>
                        <div className="flex items-center space-x-2">
                            <UserRoundSearch />
                    ...
                    <DialogContent>
                        <Form {...form}>
                            <form onSubmit={form.handleSubmit(friendRequestHandler)} className='space-y-8'>
                                <FormField control={form.control} name="email"
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormLabel>Email</FormLabel>
                                            <FormControl>
                                                <Input disabled={createFriendRequestState === 'loading'}
                                                    placeholder='friend@gmail.com' {...field} />
                                            </FormControl>
                                            <FormDescription >
